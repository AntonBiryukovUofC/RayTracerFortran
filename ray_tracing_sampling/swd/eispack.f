...
C
     ...
     X        IERR,MAXJ,MAXK,GROUP
      REAL*8 A(NM,MBW),W(M),Z(NM,M),RV(NV),RV6(N)
      REAL*8 U,V,UK,XU,X0,X1,E21,EPS2,EPS3,EPS4,NORM,ORDER,MACHEP
      REAL*8 DSQRT,DABS,DFLOAT,DSIGN
      INTEGER MIN0
C
...
C
            DO 40 I = JJ, N
               NORM = NORM + DABS(A(I,J))
               IF (E21 .GE. 0.0D0) GO TO 40
               NORM = NORM + DABS(A(IJ,KJ))
               IJ = IJ + 1
   40       CONTINUE
C
   60    CONTINUE
...
C
     ...
     X        M1,M2,M3,M4,NI,NM,NV,ITS,KJ1,M21,M31,IERR,IMULT
      REAL*8 A(NM,MB),RV(NV)
      REAL*8 F,G,Q,R,S,T,SCALE
...
C
      ...
      REAL*8 AR(NM,N),AI(NM,N),WR(N),WI(N),ZR(NM,MM),ZI(NM,MM),
     X       RM1(N,N),RM2(N,N),RV1(N),RV2(N)
      REAL*8 X,Y,EPS3,NORM,NORMV,GROWTO,ILAMBD,MACHEP,RLAMBD,UKROOT
      REAL*8 DSQRT,CDABS,DABS,DFLOAT
      LOGICAL SELECT(N)
      COMPLEX*16 Z3
      COMPLEX*16 DCMPLX
      REAL*8 DREAL,DIMAG
...
C
            DO 160 J = MP, UK
  160       X = X + CDABS(DCMPLX(AR(I,J),AI(I,J)))
C
            IF (X .GT. NORM) NORM = X
            MP = I
  180    CONTINUE
...
C     :::::::::: GROWTO IS THE CRITERION FOR GROWTH ::::::::::
         UKROOT = DSQRT(DBLE(UK))
         GROWTO = 1.0D-1 / UKROOT
  200    RLAMBD = WR(K)
         ILAMBD = WI(K)
         IF (K .EQ. 1) GO TO 280
         KM1 = K - 1
...
C
         DO 400 I = 2, UK
            MP = I - 1
            IF (CDABS(DCMPLX(RM1(I,MP),RM2(I,MP))) .LE.
     X          CDABS(DCMPLX(RM1(MP,MP),RM2(MP,MP)))) GO TO 360
C
            DO 340 J = MP, UK
               Y = RM1(I,J)
               RM1(I,J) = RM1(MP,J)
...
C
  360       IF (RM1(MP,MP) .EQ. 0.0D0 .AND. RM2(MP,MP) .EQ. 0.0D0)
     X         RM1(MP,MP) = EPS3
            Z3 = DCMPLX(RM1(I,MP),RM2(I,MP)) /
     X             DCMPLX(RM1(MP,MP),RM2(MP,MP))
            X = DREAL(Z3)
            Y = DIMAG(Z3)
...
C
  ...
  680       CONTINUE
C
  700       Z3 = DCMPLX(X,Y) / DCMPLX(RM1(I,I),RM2(I,I))
            RV1(I) = DREAL(Z3)
            RV2(I) = DIMAG(Z3)
  720    CONTINUE
...
C
         DO 820 I = 1, UK
            Z3 = DCMPLX(RV1(I),RV2(I)) / DCMPLX(X,Y)
            ZR(I,S) = DREAL(Z3)
            ZI(I,S) = DIMAG(Z3)
  820    CONTINUE
C
...
C
      ...
      REAL*8 AR(NM,IGH),AI(NM,IGH),ZR(NM,M),ZI(NM,M)
      REAL*8 XR,XI
      INTEGER INT(IGH)
C
...
C
C     ARITHMETIC IS REAL EXCEPT FOR THE REPLACEMENT OF THE ALGOL
C     PROCEDURE CDIV BY COMPLEX DIVISION USING SUBROUTINE DCMPLX.
C
C     QUESTIONS AND COMMENTS SHOULD BE DIRECTED TO B. S. GARBOW,
C     APPLIED MATHEMATICS DIVISION, ARGONNE NATIONAL LABORATORY
C
...
C
      ...
      REAL*8 SI,SR,TI,TR,XI,XR,YI,YR,ZZI,ZZR,MACHEP
      REAL*8 DABS
      COMPLEX*16 Z3
      COMPLEX*16 ZSQRT,DCMPLX
      REAL*8 DREAL,DIMAG
C     :::::::::: STATEMENT FUNCTIONS ENABLE EXTRACTION OF REAL AND
C     IMAGINARY PARTS OF DOUBLE PRECISION COMPLEX NUMBERS ::::::::::
      DREAL(Z3) = Z3
...
C     ARITHMETIC IS REAL EXCEPT FOR THE REPLACEMENT OF THE ALGOL
C     PROCEDURE CDIV BY COMPLEX DIVISION AND USE OF THE SUBROUTINES
C     ZSQRT AND DCMPLX IN COMPUTING COMPLEX SQUARE ROOTS.
C
C     QUESTIONS AND COMMENTS SHOULD BE DIRECTED TO B. S. GARBOW,
C     APPLIED MATHEMATICS DIVISION, ARGONNE NATIONAL LABORATORY
C
...
C                FOR L=EN STEP -1 UNTIL LOW D0 -- ::::::::::
  240 DO 260 LL = LOW, EN
         ...
         IF (DABS(HR(L,L-1)) + DABS(HI(L,L-1)) .LE.
...
C     :::::::::: FORM SHIFT ::::::::::
  300 IF (L .EQ. EN) GO TO 660
      ...
      Z3 = ZSQRT(DCMPLX(YR**2-YI**2+XR,2.0D0*YR*YI+XI))
      ZZR = DREAL(Z3)
      ZZI = DIMAG(Z3)
...
C
      INTEGER I,J,M,LA,MM,MP,NM,IGH,KP1,LOW,MP1
      REAL*8 AR(NM,IGH),AI(NM,IGH),ORTR(IGH),ORTI(IGH),ZR(NM,M),ZI(NM,M)
      REAL*8 H,GI,GR
C
...
C
C
      SUBROUTINE ERSEP (ERCODE, ERPNT, ERLGTH, KYWDNO, POINTR, ERTYPE,
     1   ERFATL)
C
C
C
...
C
      INTEGER
     1   ERCODE (ERLGTH), ERPNT, KYWDNO, POINTR, ERTYPE
C
      LOGICAL
     1   ERFATL
C
...
C
C
      ERCODE (ERPNT) = 100*(100*KYWDNO + POINTR) + ERTYPE
      ERFATL = ERFATL .OR. ERTYPE .EQ. 1 .OR. ERTYPE .EQ. 3
     1   .OR. ERTYPE .EQ. 4 .OR. ERTYPE .EQ. 6 .OR. ERTYPE .EQ. 7
     2   .OR. ERTYPE .EQ. 8 .OR. ERTYPE .EQ. 9 .OR. ERTYPE .EQ. 10
     3   .OR. ERTYPE .EQ. 11 .OR. ERTYPE .EQ. 70 .OR. ERTYPE .EQ. 71
     4   .OR. ERTYPE .EQ. 72 .OR. ERTYPE .EQ. 75 .OR. ERTYPE .EQ. 77
     5   .OR. ERTYPE .EQ. 78 .OR. ERTYPE .EQ. 79 .OR. ERTYPE .EQ. 80
     6   .OR. ERTYPE .EQ. 81 .OR. ERTYPE .EQ. 82 .OR. ERTYPE .EQ. 83
     7   .OR. ERTYPE .EQ. 84 .OR. ERTYPE .EQ. 85 .OR. ERTYPE .EQ. 90
     8   .OR. ERTYPE .EQ. 91 .OR. ERTYPE .EQ. 92 .OR. ERTYPE .EQ. 93
C
      IF (ERPNT .GE. ERLGTH) RETURN
C
      ERPNT = ERPNT + 1
      ERCODE (ERPNT) = -1
      RETURN
...
C
C
      DOUBLE PRECISION FUNCTION DIDFEP (VALUE)
C
      REAL*8
     1   VALUE
C
...
C
C
      INTEGER FUNCTION LKUPEP (PRLST, POINTR, PRLGTH, ALPHPR, BASE)
C
C
C
C     SPECIFICATIONS FOR PARAMETERS
...
C
      INTEGER
     1   I, INDEX (29), MCH (29), MCHLTH, MCHPTR, NOFLAG,
     2   PRBASE, PRCHR, PRLIM, PRPTR, TMPSUB, WDSCHR, WDSLIM, WDSLTH,
     3   WDSPTR (29)
C
      REAL*8
     1   WDS (29)
      LOGICAL WDSLOGICAL(8,29)
C
      INTEGER
     1   ICHREP
C
C     DECLARATIONS FOR CHARACTER COPY TO  ALPHPR
C
      REAL*8   ALPHP0
      DOUBLE PRECISION DIDFEP
C
      LOGICAL
     1   ALPHP1 (8)
C
C     FORCE ALPHP1 TO A DOUBLE-WORD BOUNDARY
      EQUIVALENCE (ALPHP0, ALPHP1 (1))
      EQUIVALENCE (WDS(1),WDSLOGICAL(1,1))
C
C
C
C
C
C     INITIALIZATION FOR TABLES
C
      DATA WDS /
     1   8HIS        , 8HUSE       , 8HNO        ,
     2   8HCOMPLEX   , 8HREAL      , 8HHERMITIA  ,
     3   8HSYMMETRI  , 8HTRIDIAGO  , 8HPOSITIVE  ,
     4   8HLARGEST   , 8HBALANCE   , 8HORTHOGON  ,
     5   8HRATQR     , 8HSMALLEST  , 8HELEMENTA  ,
     6   8HBISECT    , 8HNEGATIVE  , 8HYES       ,
     7   8HDO        , 8HFULL      , 8HGENERAL   ,
     8   8HAX=LBX    , 8HABX=LX    , 8HBAX=LX    ,
     9   8HAZ=WBZ    , 8HABZ=WZ    , 8HBAZ=WZ    ,
     O   8HPACKED    , 8HUNITARY
     1   /
C
      DATA INDEX /
     1             +1,           +1,           -1,
...
C
C
C     THE ABOVE DECLARATIONS MUST CORRESPOND TO INDEX (WDSLTH),
C        MCH (WDSLTH), WDSPTR (WDSLTH), AND WDS (WDSLTH).
C
      WDSLTH = (29)
      WDSLIM = 8
      PRLIM = 16
...
C
  105 CONTINUE
      ...
      PRBASE = PRLST (POINTR)
C
C     SAVE THE FIRST EIGHT CHARACTERS OF THE ALPHANUMERIC PARAMETER
C        IN  ALPHPR  IN CASE AN ERROR MESSAGE MUST BE PRINTED.  THE
C        CHARACTERS ARE COPIED ONE BYTE AT A TIME TO AVOID SPECIFICATION
C        EXCEPTIONS WHICH WOULD ARISE ON IBM S/360 IF STRINGS WERE
C        NOT COMPILED ON A DOUBLE WORD BOUNDARY.  THE FOLLOWING LOOP
C        AND ITS RELATED DECLARATIONS AND EQUIVALENCE STATEMENT
C        CAN BE REPLACED ON OTHER MACHINES BY A SIMPLE ASSIGNMENT TO
C        ALPHPR.
C
      DO 101 I = 1, 8
         TMPSUB = PRBASE + I - 1
         ALPHP1 (I) = BASE (TMPSUB)
  101 CONTINUE
      ALPHPR = DIDFEP (ALPHP0)
      PRPTR = 0
C
      DO 102 I = 1, WDSLTH
         WDSPTR (I) = 0
...
C
  104 CONTINUE
         MCHLTH = MCHPTR
         MCHPTR = 0
         PRCHR = ICHREP (BASE (PRBASE), PRPTR, PRLIM)
         IF (PRCHR .EQ. -1) RETURN
C
         DO 103 I = 1, MCHLTH
            TMPSUB = MCH (I)
      WDSCHR = ICHREP (WDSLOGICAL(1,TMPSUB), WDSPTR (TMPSUB), WDSLIM)
            IF (PRCHR .NE. WDSCHR) GO TO 103
            MCHPTR = MCHPTR + 1
            MCH (MCHPTR) = MCH (I)
  103    CONTINUE
      IF (MCHPTR .GE. 2) GO TO 104
      IF (MCHPTR .EQ. 0) RETURN
      TMPSUB = MCH (1)
      NOFLAG = NOFLAG*INDEX (TMPSUB)
      LKUPEP = NOFLAG
      IF (IABS (NOFLAG) .NE. 1) RETURN
      POINTR = POINTR + 1
      IF (POINTR .LE. PRLGTH) GO TO 105
      RETURN
C
      END
C
...
C
C
      INTEGER FUNCTION ICHREP (STRING, CHRNO, STRLTH)
C
C
C
C     SPECIFICATIONS OF PARAMETERS
C
      LOGICAL
     1   STRING (1)
C
...
C
C
C     DECLARATIONS FOR CHARACTER INSERTION AS INTEGER
C
      INTEGER
     1   CHRCOD, FIDFEP
C
     ...
     1   CHREQV (4)
C
C     FORCE CHREQV TO A FULL-WORD BOUNDARY
      EQUIVALENCE (CHRCOD, CHREQV (1))
      DATA BLANK /64/
C
C
C
C
C
C     THIS FUNCTION RETURNS THE MACHINE CHARACTER CODE FOR THE NEXT
C        NON-BLANK CHARACTER IN STRING FOLLOWING CHRNO, PROVIDED
C        ONE EXISTS FOR CHRNO .LE. STRLTH.  OTHERWISE, IT RETURNS -1.
C        THE MACHINE CHARACTER CODE IS INSERTED AS THE RIGHTMOST
C        BITS OF A MACHINE INTEGER.  ON EXIT, CHRNO POINTS TO THE
C        CHARACTER WHOSE CODE IS RETURNED OR TO STRLTH+1.  ICHREP
C        MIGHT WELL BE WRITTEN IN MACHINE LANGUAGE.
C
C
      CHRCOD = 0
      ICHREP = -1
C
  101 CONTINUE
      CHRNO = CHRNO + 1
      IF (CHRNO .GT. STRLTH) RETURN
      CHREQV (4) = STRING (CHRNO)
C     FORCE COMPILER TO MAKE EQUIVALENCED VARIABLES MATHEMATICALLY EQUAL
C      CHRCOD = FIDFEP (CHREQV (1))
      CHRCOD = FIDFEP (CHRCOD)
      IF (CHRCOD .EQ. BLANK) GO TO 101
      ICHREP = CHRCOD
      RETURN
C
      END
...
C     :::::::::: LOOK FOR SMALL SUB-DIAGONAL ELEMENT ::::::::::
  105    DO 110 M = L, N
            IF (M .EQ. N) GO TO 120
            IF (DABS(RV1(M)) .LE. MACHEP * (DABS(W(M)) + DABS(W(M+1))))
     X         GO TO 120
...
C
  700       IF (I .LT. N1) GO TO 710
            ...
            GO TO 715
  710       T = RM1(I,I+2)
  715       Z3 = DCMPLX(X,Y) / DCMPLX(RM1(I,I),T)
            RV1(I) = DREAL(Z3)
            RV2(I) = DIMAG(Z3)
  720    CONTINUE
...
C
         DO 780 I = 1, UK
            ...
            IF (ILAMBD .NE. 0.0D0) X = CDABS(DCMPLX(RV1(I),RV2(I)))
            IF (NORMV .GE. X) GO TO 760
            NORMV = X
            J = I
  760       NORM = NORM + X
...
C
         DO 820 I = 1, UK
            ...
            Z(I,S) = RV1(I) * X
            GO TO 820
  800       Z3 = DCMPLX(RV1(I),RV2(I)) / DCMPLX(X,Y)
            Z(I,S-1) = DREAL(Z3)
            Z(I,S) = DIMAG(Z3)
  820    CONTINUE
C
...
C
      INTEGER I,J,M,LA,MM,MP,NM,IGH,KP1,LOW,MP1
      REAL*8 A(NM,IGH),ORT(IGH),Z(NM,M)
      REAL*8 G
C
...
C
      ...
      REAL*8 D(N),E(N),E2(N),W(N),BD(N)
      REAL*8 F,P,Q,R,S,EP,QP,ERR,TOT,EPS1,DELTA,MACHEP
      REAL*8 DABS,DMIN1
      INTEGER IND(N)
      LOGICAL TYPE
C
...
C
      DO 100 I = 1, N
         P = Q
         IF (I .EQ. 1) GO TO 60
         IF (P .GT. MACHEP * (DABS(D(I)) + DABS(D(I-1)))) GO TO 80
   60    E2(I) = 0.0D0
   80    BD(I) = E2(I)
...
C                TO REDUCE THE INCIDENCE OF UNDERFLOWS ::::::::::
  190    IF (K .EQ. N) GO TO 210
         K1 = K + 1
         DO 200 J = K1, N
            IF (BD(J) .LE. (MACHEP*(W(J)+W(J-1))) ** 2) BD(J) = 0.0D0
  200    CONTINUE
C
  210    F = BD(N) / DELTA
         QP = DELTA + F
...
C
      ...
      REAL*8 A(NM,MB),W(N),Z(NM,N),FV1(N),FV2(N)
      LOGICAL TF
C
C     THIS SUBROUTINE CALLS THE RECOMMENDED SEQUENCE OF
C     SUBROUTINES FROM THE EIGENSYSTEM SUBROUTINE PACKAGE (EISPACK)
...
C
      INTEGER N,NM,IERR,MATZ
      REAL*8 A(NM,3),W(N),Z(NM,N),FV1(N)
C
C     THIS SUBROUTINE CALLS THE RECOMMENDED SEQUENCE OF
C     SUBROUTINES FROM THE EIGENSYSTEM SUBROUTINE PACKAGE (EISPACK)
C     TO FIND THE EIGENVALUES AND EIGENVECTORS (IF DESIRED)
...
C
     ...
     X        IERR,GROUP,ISTURM
      REAL*8 D(N),E(N),E2(N),W(MM),Z(NM,MM),
     X       RV1(N),RV2(N),RV3(N),RV4(N),RV5(N),RV6(N)
      REAL*8 U,V,LB,T1,T2,UB,UK,XU,X0,X1,EPS1,EPS2,EPS3,EPS4,
     X       NORM,MACHEP
      REAL*8 DSQRT,DABS,DMAX1,DMIN1,DFLOAT
C
C     THIS ROUTINE IS A TRANSLATION OF THE ALGOL PROCEDURE TRISTURM
C     BY PETERS AND WILKINSON.
C     HANDBOOK FOR AUTO. COMP., VOL.II-LINEAR ALGEBRA, 418-439(1971).
C
...
C./ ENDUP
C      FUNCTION ZSQRT(Z)
C      COMPLEX*16 ZSQRT,Z
C      REAL*8 A,B
C      A=CDABS(Z)
C      B=DREAL(Z)
CC  DEC CONVENTIONS:
CC  ENSURE THAT REAL PART IS POSITIVE-DEFINITE
C      IF(DIMAG(Z).NE.0.D0) THEN
C      ZSQRT=DCMPLX(DSQRT((A+B)/2.D0),
C     1             DSIGN(DSQRT((A-B)/2.D0),DIMAG(Z)))
C      ELSEIF(B.GT.0.D0) THEN
C      ZSQRT=DCMPLX(DSQRT(DREAL(Z)),0.D0)
C      ELSEIF(B.LT.0.D0) THEN
CC  IF REAL PART OF ZSQRT IS ZERO, MAKE IMAG PART POSITIVE
C      ZSQRT=DCMPLX(0.D0,DSQRT(-DREAL(Z)))
C      ELSE
C      ZSQRT=DCMPLX(0.D0,0.D0)
C      ENDIF
C      RETURN
C      END
      SUBROUTINE UTL1EP (NM, N, DEST1, SRC1)
C
C
C     EISPAC EXECUTION PHASE UTILITY ROUTINE
C
C
      INTEGER
     1   N, NM
