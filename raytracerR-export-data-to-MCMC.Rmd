...
NSrc <- 10
set.seed(10)
src.df <- tibble(z=runif(NSrc,min=200,max=6500),x=runif(NSrc,min=10,max=4500),y=runif(NSrc,min=10,max=4500))
srcd <- src.df$z
srco <- sqrt(src.df$x**2+src.df$y**2)
src.to.write <- tibble(offset = srco,depth=srcd)
```
...
#
sd = 0.016
timeP$timeP <- timeP$timeP + rnorm(NSrc,mean=0,sd=sd)
...
true.model = tibble(variable =c(col.names.voro[order(col.names.voro)],"SdRT"),value = c(v,c(0,d),sd))
...
p <- ggplot(data = chain.melted,aes(x=value,group = variable)) + geom_histogram(bins=100) + facet_wrap(~variable,scales = 'free') + geom_vline(aes(xintercept=value),data = true.model,col = 'salmon',size=1.2)
print(p)
